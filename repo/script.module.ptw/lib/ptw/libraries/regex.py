...
profile = functions_dir = xbmcvfs.translatePath(
...
def insert(data):
    try:
        ...
        for i in data:
            try:
                dbcur.execute(
                    "INSERT INTO regex Values (?, ?)", (i["regex"], i["response"])
                )
            except:
                pass
        dbcon.commit()
...
def resolve(regex):
    try:
        ...
        cddata = re.compile("<\!\[CDATA\[(.+?)\]\]>", re.MULTILINE | re.DOTALL).findall(
            regex
        )
        for i in cddata:
        ...
        regexs = re.compile("<regex>(.+?)</regex>", re.MULTILINE | re.DOTALL).findall(
            regexs
        )
        regexs = [
            re.compile("<(.+?)>(.*?)</.+?>", re.MULTILINE | re.DOTALL).findall(i)
            for i in regexs
        ]
        ...
        if k in regexs:
            ...
            if cookieJarParam:
                if cookieJar is None:
                    ...
                    if cookie_jar_file:
                        saveCookieJar(
                            cookieJar, cookie_jar_file
                        )  # import cookielib  # cookieJar = cookielib.LWPCookieJar()  # print 'cookieJar new',cookieJar
            ...
            if (
                m["page"]
                and m["page"] in cachedPages
                and not "ignorecache" in m
                and forCookieJarOnly == False
            ):
                ...
                if m["page"] and not m["page"] == "" and m["page"].startswith("http"):
                    ...
                    if "rawpost" in m:
                        post = m[
                            "rawpost"
                        ]  # if '$LiveStreamRecaptcha' in post:  #    (captcha_challenge,catpcha_word,idfield)=processRecaptcha(m['page'],cookieJar)  #    if captcha_challenge:  #       post=post.replace('$LiveStreamRecaptcha','&manual_recaptcha_challenge_field='+captcha_challenge+'&recaptcha_response_field='+catpcha_word+'&id='+idfield)
                    link = ""
                    try:
                    ...
                    if m["page"].startswith("$pyFunction:"):
                        val = doEval(
                            m["page"].split("$pyFunction:")[1], "", cookieJar, m
                        )
                        if forCookieJarOnly:
                            return cookieJar  # do nothing
                        link = val
            ...
            if not m["expres"] == "":
                ...
                if "$LiveStreamCaptcha" in m["expres"]:
                    ...
                    m["expres"].startswith("$pyFunction:")
                    ...
                    if m["expres"].startswith("$pyFunction:"):
                        val = doEval(
                            m["expres"].split("$pyFunction:")[1], link, cookieJar, m
                        )
                    else:
...
def get_unpacked(page_value, regex_for_text="", iterations=1, total_iteration=1):
    try:
        ...
        if regex_for_text and len(regex_for_text) > 0:
            try:
                page_value = re.compile(regex_for_text).findall(page_value)[
                    0
                ]  # get the js variable
            except:
                return "NOTPACKED"
    ...
    if "sav1live.tv" in page_value:
        page_value = page_value.replace(
            "sav1live.tv", "sawlive.tv"
        )  # quick fix some bug somewhere
...
def unpack(sJavascript, iteration=1, totaliterations=2):
    ...
    if iteration >= totaliterations:
        ...
        return unpack(
            sUnpacked1, iteration + 1
        )  # .replace('\\', ''),iteration)#.replace('\\', '');#unpack(sUnpacked.replace('\\', ''))
    ...
    if len(splits) > 1:
        ...
        for current_split in splits:
            ...
            if current_index == len(splits) - 1:
                ...
                if (
                    len(current_split) == 0
                ):  # if blank check next one with current split value
                    if (
                        len(splits[current_index + 1]) == 0
                        and word_to_find[0].lower()
                        not in "abcdefghijklmnopqrstuvwxyz1234567890_"
                    ) or (
                        len(splits[current_index + 1]) > 0
                        and splits[current_index + 1][0].lower()
                        not in "abcdefghijklmnopqrstuvwxyz1234567890_"
                    ):  # first just just check next
                        val = replace_with
                # not blank, then check current endvalue and next first value
                else:
                    if (
                        splits[current_index][-1].lower()
                        not in "abcdefghijklmnopqrstuvwxyz1234567890_"
                    ) and (
                        (
                            len(splits[current_index + 1]) == 0
