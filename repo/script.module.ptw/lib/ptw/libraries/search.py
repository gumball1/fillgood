...
import xbmcplugin
import xbmcvfs
from xbmcplugin import ListItem
...
class Search(object):
    ...
    def __init__(
        ...
        self._spath = xbmcvfs.translatePath(
            "special://profile/addon_data/%s/search" % self._addon.getAddonInfo("id")
        )
        self.history = []
        self.loadHistory()
    ...
    def addMenuItem(self, name, iconImage=None, folder=True, menu=True, **kwargs):
        ...
        # general menu item
        # liz = ListItem(title, iconImage="DefaultFolder.png", thumbnailImage=iconImage)
        # liz.setInfo(type="Video", infoLabels={"Title": title})
        url = self.buildPluginUrl(name=name, **kwargs)
        xbmc.log(
            'SEARCH: create menu item %s, query:"%s", url:"%s"'
            % (name, kwargs.get("query"), url)
        )
        li = ListItem(
            kwargs.get("title", ""),
        ...
        xbmcplugin.addDirectoryItem(
            handle=self._addonHandle, url=url, listitem=li, isFolder=folder
        )
