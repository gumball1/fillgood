...
_check_icon = os.path.join(
    _path, "resources", "check.png"
)  # Don't decode _path to utf-8!!!
...
class SelectDialog(pyxbmct.AddonDialogWindow):
    def __init__(
        ...
        self.column_items.append([])
        self.set_column(
            2, 0, "Gatunek:", self.genre[0 : (len(self.genre) / 3) - 1] or []
        )
        ...
        self.set_column(
            ...
            0,
            "",
            self.genre[(len(self.genre) / 3) - 1 : ((len(self.genre) / 3) * 2) - 2]
            or [],
        )
        ...
        self.set_column(
            ...
            "",
            self.genre[
                ((len(self.genre) / 3) * 2) - 2 : ((len(self.genre) / 3) * 3) - 3
            ]
            or [],
        )
        ...
        self.set_column(
            ...
            0,
            "",
            self.genre[((len(self.genre) / 3) * 3) - 3 : len(self.genre)] or [],
        )
    ...
    def set_column(self, column=0, row_offset=0, header="", items=None):
        if items is None:
            items = []
        if header is not "":
            typelabel = pyxbmct.Label(header, alignment=4)
            self.placeControl(
                typelabel, row_offset, column, rowspan=1, columnspan=1, pad_x=1, pad_y=1
            )
        # print items
    ...
    def check_uncheck(self, btn):
        ...
        if button.getLabel() in self.sort:
            for i in range(7, 11):
                ...
                self.column_items[0][i].setLabel(
                    label, font="fontContextMenu", textColor=_white, focusedColor=_white
                )
                try:
        ...
        if button.getLabel() in self.genre_opt:
            for i in range(5, 7):
                ...
                self.column_items[1][i].setLabel(
                    label, font="fontContextMenu", textColor=_white, focusedColor=_white
                )
                try:
                    self.selected.remove(label)
            ...
            button.setLabel(
                button.getLabel(),
                font="fontContextMenu",
                textColor=_yellow,
                focusedColor=_yellow,
            )
