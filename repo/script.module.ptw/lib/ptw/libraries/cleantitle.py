...
def getsearch(title, preserve=""):
    if title is None:
        return
    title = title.lower()
    title = re.sub(r"&#(\d+);", "", title)  # encje numeryczne w postaci dziesiętnej
    title = re.sub("(&#[0-9]+)([^;^0-9]+)", "\\1;\\2", title)  # też coś z encjami
    title = title.replace("&quot;", '"').replace("&amp;", "&")
    title = re.sub(r"\\|!|\[|\]|–|;|,|\*|\?|\"|<|>|\|", " ", title)  # potem się ewentualnie posprząta, a czasami spacja się przydaje
...
def scene_tvtitle(title, year, season, episode):
    ...
    if (
        title
        in ["The Haunting", "The Haunting of Bly Manor", "The Haunting of Hill House"]
        and year == "2018"
    ):
        if season == "1":
            ...
            season = "1"
    if (
        title in ["Cosmos", "Cosmos A Spacetime Odyssey", "Cosmos Possible Worlds"]
        and year == "2014"
    ):
        if season == "1":
