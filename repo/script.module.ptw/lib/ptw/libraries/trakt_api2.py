...
class TraktAPI2:
    ...
    def get_token(self, pin=None):
        ...
        if pin:
            ...
            if refresh_token:
                ...
                raise TraktError(
                    "Can not refresh trakt token. Trakt reauthorizion required."
                )
    ...
    def show_list(self, slug, section, username=None, auth=True, cached=True):
        ...
        list_data = self.__call_trakt(
            url, params=params, auth=auth, cache_limit=cache_limit, cached=cached
        )
        return [
            item[item["type"]]
    ...
    def show_watchlist(self, section):
        ...
        params = {"extended": "full,images"}
        cache_limit = self.__get_cache_limit(
            TRAKT_SECTIONS[section], "watchlisted_at", cached=True
        )
    ...
    def get_liked_lists(self, page=None, cached=True):
        ...
        return self.__call_trakt(
            url, params=params, cache_limit=cache_limit, cached=cached
        )
    ...
    def get_premieres(self, start_date=None, days=None, cached=True):
        ...
        return self.__call_trakt(
            url, params=params, auth=False, cache_limit=24, cached=cached
        )
    ...
    def get_my_calendar(self, start_date=None, days=None, cached=True):
        ...
        return self.__call_trakt(
            url, params=params, auth=True, cache_limit=24, cached=cached
        )
    ...
    def get_bookmark(self, show_id, season, episode):
        ...
        for bookmark in response:
            if not season or not episode:
                if (
                    bookmark["type"] == "movie"
                    and int(show_id) == bookmark["movie"]["ids"]["trakt"]
                ):
                ...
                if (
                    ...
                    and int(show_id) == bookmark["show"]["ids"]["trakt"]
                    and bookmark["episode"]["season"] == int(season)
                    and bookmark["episode"]["number"] == int(episode)
                ):
                    return bookmark["progress"]
    ...
    def __get_cache_limit(self, media, activity, cached):
        if cached:
            ...
            log_utils.log(
                "Now: %s Last: %s Last TS: %s Cache Limit: %.2fs (%.2fh)"
                % (
                    time.time(),
                    utils2.iso_2_utc(activity),
                    activity,
                    cache_limit,
                    cache_limit / 60 / 60,
                ),
    ...
    def __call_trakt(
            ...
            try:
                ...
                if isinstance(e, urllib2.HTTPError):
                    if e.code in TEMP_ERRORS:
                        control.infoDialog(
                            "Temporary Trakt Error: " + str(e), "Trakt ERROR"
                        )
                        raise TransientTraktError("Temporary Trakt Error: " + str(e))
                        ...
                        if e.info().getheader("X-Private-User") == "true":
                            control.infoDialog(
                                "Object is No Longer Available (%s)" % e.code,
                                "Trakt ERROR",
                            )
                        ...
                        # auth failure retry or a token request
                            ...
                            control.infoDialog(
                                "Trakt Call Authentication Failed (%s)" % e.code,
                                "Trakt ERROR",
                            )
                            raise TraktAuthError(
                                "Trakt Call Authentication Failed (%s)" % e.code
                        ...
                        # first try token fail, try to refresh token
                            ...
                            control.set_setting(
                                "trakt_oauth_token", result["access_token"]
                            )
                            control.set_setting(
                                "trakt_refresh_token", result["refresh_token"]
                            )
                            auth_retry = True
                    elif e.code == 404:
                        control.infoDialog(
                            "Object Not Found (%s)" % e.code, "Trakt ERROR"
                        )
        ...
        try:
            ...
            if "x-sort-by" in res_headers and "x-sort-how" in res_headers:
                js_data = utils2.sort_list(
                    res_headers["x-sort-by"], res_headers["x-sort-how"], js_data
                )
        except ValueError:
            js_data = ""
            if result:
