...
def get(name, imdb, season, episode):
    try:
        ...
        if data["status"] != "200 OK":
            ...
            control.infoDialog(
                "Open Subtitles Error: " + str(data["status"]),
        ...
        if not (season == "" or episode == ""):
            ...
            result = server.SearchSubtitles(
                token, [{"sublanguageid": sublanguageid, "imdbid": imdbid}]
            )["data"]
            try:
        ...
        for lang in langs:
            filter += [
                ...
                and any(x in i["MovieReleaseName"].lower() for x in fmt)
            ]
            filter += [
                i
                for i in result
                if i["SubLanguageID"] == lang
                and any(x in i["MovieReleaseName"].lower() for x in quality)
            ]
