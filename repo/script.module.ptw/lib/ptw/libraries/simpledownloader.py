...
def download(name, image, url):
    ...
    sysheaders = urlparse.quote_plus(json.dumps(headers))

    sysurl = urlparse.quote_plus(url)

    systitle = urlparse.quote_plus(name)

    sysimage = urlparse.quote_plus(image)

    sysdest = urlparse.quote_plus(dest)
...
def doDownload(url, dest, title, image, headers):
    headers = json.loads(urlparse.unquote_plus(headers))

    url = urlparse.unquote_plus(url)

    title = urlparse.unquote_plus(title)

    image = urlparse.unquote_plus(image)

    dest = urlparse.unquote_plus(dest)
    ...
    while True:
        ...
        if percent >= notify:
            xbmc.executebuiltin(
                ...
                % (
                    title + " - Download Progress - " + str(percent) + "%",
                    dest,
                    10000,
                    image,
                )
