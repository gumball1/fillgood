...
class trailer:
    ...
    def play(self, name="", url="", windowedtrailer=0):
        try:
            # if not url or not url.startswith(self.youtube_watch.replace("%s")):
            url = self.worker(name, url)
            if not url:
                control.log('[trailer.py] No url to play')
                return
            # control.log(f"[FanFilm] [trailer.py] {url=} {name=}", 1)
            ...
            item = control.item(label=name, path=url)  # utworzenie Listitem do odtwarzania
            ...
            control.resolve(handle=int(sys.argv[1]), succeeded=True, listitem=item)  # start playing
    ...
    def search(self, url):
        ytbso = YoutubeSearch(url, max_results=1)
        try:
            results = json.loads(ytbso.to_json())
