...
class source:
    ...
    def __init__(self):
        ...
        if control.settings.getBool("tb7.wiele_serwerow"):
        ...
        self.user_name = control.settings.getString("tb7.username")
        self.user_pass = control.settings.getString("tb7.password")
        self.headers = {
    ...
    def search(self, title, localtitle, year="", episode="", aliases=None, premiered=""):
        ...
        try:
            ...
            # """ serwis tb7 bardzo długo kazał czekać na wyniki, więc wymyśliłem
            ...
            last_search_info = cache.cache_get("tb7_last_search_info", control.sourcescacheFile)
            if last_search_info:
            ...
            def check_if_finished():
                results_cache_fin = cache.cache_get("tb7_results_finished", control.sourcescacheFile)
            ...
            if results_cache_fin:
                ...
                if klucz not in results_cache_val:
                    ...
                    cache.cache_insert("tb7_results", "", control.sourcescacheFile)
                    cache.cache_insert("tb7_results_finished", "", control.sourcescacheFile)
                    cache.cache_insert("tb7_raw_results", "", control.sourcescacheFile)
                    #cache.cache_insert("tb7_trash", "", control.sourcescacheFile)
                else:
                    # tytuły się zgadzają
            ...
            now_extended_search = control.settings.getBool("tb7.extended_search")
            if dont_request:
                if last_extended_search is False and now_extended_search is True:
                    dont_request = False
                    fflog(f'{dont_request=}', 0)

            def korekty_dla_wyszukiwarki_tb7(title):
            ...
            if dont_request:
                for title1 in titles:
                    self.titles = [korekty_dla_wyszukiwarki_tb7(title1)] + self.titles
                self.titles = list(dict.fromkeys(self.titles))
            ...
            if dont_request:
                ...
                cache.cache_insert(
                    "tb7_results_finished",
                    repr({klucz: 0}),
                    control.sourcescacheFile
                )
                saved_to_cache = True

                aliases3 = [korekty_dla_wyszukiwarki_tb7(a) for a in aliases2]  # przygotowanie do wybrania lepszej frazy do wyszukiwarki
                # fflog(f'{aliases3=}')
            ...
            #for title1 in [title2]:
                ...
                if isinstance(title,str):
                    ...
                    if not customTitles:
                        title = korekty_dla_wyszukiwarki_tb7(title)
                    ...
                    if not customTitles:
                        ...
                        if title1 in titles0:
                            wspolny_prefiks = znajdz_wspolny_prefiks_wyrazowy([korekty_dla_wyszukiwarki_tb7(t) for t in titles if isinstance(t,str)])
                ...
                if ( title1 == titles[-1]  # ostatni
                    ...
                    # wyszukiwarka tb7 zwraca maksymalnie tylko 4 podstrony (jakieś ograniczenie)
            ...
            if saved_to_cache:
                ...
                cache.cache_insert("tb7_last_search_info", repr({
                    "total_time": max(1, tt),
                    "klucz": klucz,
                    "titles_r": titles_r,
            ...
            cache.cache_insert("tb7_results_finished", "", control.sourcescacheFile)  # problem, jak baza zablokowana
        ...
        if self.trash2:
            ...
            # fflog(f'{len(trash2)=}')
            cache.cache_insert("tb7_trash", repr(trash2), control.sourcescacheFile)
    ...
    def get_pages_content(self, page, year, title="", episode="", premiered="", aliases=None, rows=None):
        ...
        if not rows:
            if page:
                ...
                results_cache = cache.cache_get("tb7_raw_results", control.sourcescacheFile)
        ...
        try:
            for row in rows:
                try:
                    ...
                    # if not size or "tb7.pl/pobieramy/" in link:  # to niweluje cały zamysł, bo taki link może być dla pewnych szczególnych pozycji
            ...
            cache.cache_insert("tb7_src", repr(src), control.sourcescacheFile)
            # w bazie cache będzie key(tb7_src), value(słownik w formie stringa)
    ...
    def resolve(self, url, buy_anyway=False, specific_source_data=None):
        ...
        if not specific_source_data:
            ...
            sources_data = cache.cache_get("tb7_src", control.sourcescacheFile)
            if sources_data:
            ...
            if '/pobieramy/' in link:
                ...
                if response.status_code == 200:
                    ...
                    if 'dla podanego linka Premium' in response.text:
                        control.dialog.ok('tb7', 'Wykorzystano limit połączeń dla tego źródła.')
                    else:
                        control.dialog.ok('tb7', 'Ważność linku wygasła.')
                        control.window.clearProperty('imdb_id')  # aby odświeżyć listę źródeł
                        control.window.setProperty('clear_SourceCache_for', control.window.getProperty('clear_SourceCache_for') + ',tb7')  # jak ktoś używa cache źródeł
                    # control.execute('Dialog.Close(notification,true)')
                    return None
                ...
                if response.status_code == 403:
                    ...
                    control.infoDialog(f'Serwer zwrócił błąd nr {response.status_code}', 'tb7', 'ERROR', 4000)
                    control.sleep(4000)
        ...
        auto_purchase = control.settings.getBool("autotb7")
        if not auto_purchase:
            ...
            if True:
                ...
                try:
                    ...
                    try:
                        cp = control.settings.getString("tb7.color.identify2")
        ...
        for link in links:
            ...
            response = self.session.post("https://tb7.pl/mojekonto/sciagaj", data=data_step1, headers=self.headers).text
        ...
        if ' value="Wgraj linki"' not in response:
            mnoga = len(links) > 1
            control.infoDialog((f"Wystąpił błąd. \nTa pozycja ma nieaktywn{'e' if mnoga else 'y'} link{'i' if mnoga else ''}."), 'tb7', 'ERROR')
        ...
        # do testów
        # return (False if not control.dialog.yesno("tb7", (f"Aktywny link to \n[LIGHT][B][I]{self.prepare_filename_to_display(active_url)}[/I][/B][/LIGHT]\nCzy kontynuować?") ) else True)

        # krok 2 - próba dodania źródła do biblioteki
        data_step2 = {"0": "on", "step": "2"}
        response = self.session.post("https://tb7.pl/mojekonto/sciagaj", data=data_step2, headers=self.headers).text
        ...
        try:
            ...
            if "Nieaktywne linki" in response:
                control.dialog.notification("tb7", "Link okazał się nieaktywny")
                fflog(f'jednak zły {link=}')
            else:
                control.infoDialog("Wystąpił jakiś błąd. \nMoże brak wymaganego transferu?", "tb7", "ERROR")
            # control.dialog.ok('Brak środków', f'Brak wystarczającego transferu. \n[COLOR gray](aktualnie posiadasz [B]{remaining_limit}[/B])[/COLOR]')
            fflog_exc()
            return None

        # ewnentualne zapisanie informacji, aby następnym razem wiedzieć z jakim linkiem powiązać plik na koncie
        if control.settings.getBool("tb7.use_web_notebook_for_history") and specific_source_data:
            # nazwa pliku
            ...
            while len(repr(notes_list)) >= 5000:  # limit narzucony przez serwis tb7.pl
                del notes_list[-1]
            now0 = int(time.time())
            self.session.post(
            ...
            now1 = int(time.time())
            if (now1 - now0) > 5:
                fflog(f'!wysłanie historii plików do "tb7.pl/mojekonto/notes" zajęło {(now1 - now0)} sek.')
    ...
    def files_on_user_account(self, mode=1, from_cache=None):
        ...
        if control.settings.getBool("tb7.use_web_notebook_for_history"):  # a może zawsze sprawdzać?
            result = cache.cache_get(f"tb7.pl_{self.mynotepad_link}", control.sourcescacheFile)
            if result:
                if (
                    from_cache
            ...
            if not result:
                ...
                cache.cache_insert(f"tb7.pl_{self.mynotepad_link}", notes_page_content, control.sourcescacheFile)
        ...
        result = cache.cache_get(f"tb7.pl_{self.mylibrary_link}", control.sourcescacheFile)
        if result:
            if (
                from_cache
                or from_cache is not False
        ...
        if not result:
            ...
            cache.cache_insert(f"tb7.pl_{self.mylibrary_link}", result, control.sourcescacheFile)
        ...
        if control.settings.getBool("tb7.use_web_notebook_for_history"):
            # skrócenie linków
        ...
        if not on_account:  # teraz test z nazwami plików
                ...
                for item in biblio_links:  # element z "Historii linków"
                    ...
                    for filename in filenames:
                        ...
                        if unescape(unquote(filename)) in unescape(unquote(item)):
                            ...
                            # (strona tb7 nie podaje w tej zakładce ani serwera ani rozmiaru)
                            on_account_link = item_org[0]
                            on_account_expires = item_org[1]
                            on_account = True
            ...
            if not on_account and filenames:
                for item in biblio_links:  # element z "Historii linków"
                    item_org = item
                    item = item[0]
                    if item[-3:] not in self.VIDEO_EXTENSIONS:  # gdy przez tb7 ucięty, jak dla "Flip i Flap Utopia"
    ...
    def login(self):
        ...
        try:
            cookies = cache.cache_get("tb7_cookie", control.sourcescacheFile)["value"]
        except Exception:
            cookies = ""
        self.headers.update({"Cookie": cookies})

        result = cache.cache_get("tb7.pl_glowna", control.sourcescacheFile)
        if result:
        ...
        if not result:
            ...
            cache.cache_insert("tb7.pl_glowna", result, control.sourcescacheFile)
        if self.user_name in result:
            ...
            if self.user_name and self.user_pass:
                ...
                if self.user_name in result:
                    ...
                    cache.cache_insert("tb7_cookie", cookies, control.sourcescacheFile)
                    self.headers.update({"Cookie": cookies})
                    cache.cache_insert("tb7.pl_glowna", result, control.sourcescacheFile)
                else:
                    fflog("logowanie nieudane!")
                    control.infoDialog('logowanie nieudane', 'tb7', 'ERROR')
            else:
                fflog("BRAK danych do zalogowania! - sprawdź ustawienia")
                control.infoDialog('BRAK danych do zalogowania! - sprawdź ustawienia', 'tb7', 'ERROR')
