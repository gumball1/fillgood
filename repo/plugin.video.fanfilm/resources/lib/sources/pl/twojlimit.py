...
class source:
    def __init__(self):
        ...
        self.req = requests
        self.user_name = control.settings.getString("twojlimit.username")
        self.user_pass = control.settings.getString("twojlimit.password")
        self.trash = []
        #self.trash2 = []
        self.titles = []
    ...
    def login(self):
        ...
        self.authtoken = ""
        get_auth = cache.cache_get("twojlimit_authtoken", control.sourcescacheFile)
        ...
        if self.authtoken == "":
            if self.user_name and self.user_pass:
                ...
                if "authtoken" in response:
                    ...
                    cache.cache_insert("twojlimit_authtoken", self.authtoken, control.sourcescacheFile)
                    return True
                if "error" in response and response["message"] != "":
                    xbmcgui.Dialog().notification("twojlimit - Błąd", str(response["message"]), xbmcgui.NOTIFICATION_ERROR)
            else:
                xbmcgui.Dialog().notification("twojlimit - Błąd", "Brak danych logowania - sprawdź ustawienia", xbmcgui.NOTIFICATION_ERROR)
        return False
    ...
    def search(self, title, localtitle, year="", episode="", premiered="", aliases=None, recurrency=None):
        """ pobiera wyniki z API serwisu twojlimit """
        try:
            ...
            for title in titles:
                ...
                if "error" in search and search["message"] != "":
                    cache.cache_insert("twojlimit_authtoken", "", control.sourcescacheFile)
    ...
    def resolve(self, url):
        ...
        try:
            ...
            autotwojlimit = control.settings.getBool("autotwojlimit")
            ...
            if "file" in response:
                ...
                if "url" in file:
                    ...
                    control.window.setProperty('clear_SourceCache_for', control.window.getProperty('clear_SourceCache_for') + ',twojlimit')  # jak ktoś używa cache źródeł
            ...
            xbmcgui.Dialog().notification(f"twojlimit - {error}", message, xbmcgui.NOTIFICATION_ERROR)
