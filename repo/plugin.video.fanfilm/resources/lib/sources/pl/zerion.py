...
import requests
import urllib.parse
from ptw.libraries import control, cache, cleantitle, source_utils
...
class source:
    ...
    def resolve(self, url, content_type=""):
        ...
        some_data = cache.cache_get("zerion", control.sourcescacheFile)
        ...
        try:
            cookies = cache.cache_get("zerion_cookie", control.sourcescacheFile)["value"]
        except Exception:
            cookies = ""
    ...
    def search_content(self, content_type, titles, aliases=None, year=""):
        ...
        with requests.Session() as session:
            ...
            for title in titles:
                ...
                if year:
                    results = [r for r in results if 
                               r["year"] == year
                               or r["year"] == str(int(year)-1)  # na zapas
                               or r["year"] == str(int(year)+1)  # wyjątek dla "Lost"
                              ]
                # fflog(f'{len(results)=}  {results=}')
            ...
            cache.cache_insert("zerion_cookie", cookies, control.sourcescacheFile)
    ...
    def get_sources(self, video_results, session, content_type):
        ...
        if video_results:
            ...
            # if csrf_token:
            cache.cache_insert("zerion", repr({"csrf_token": csrf_token, "content_type": content_type}), control.sourcescacheFile)
            # control.sleep(0.6 * 1000)
