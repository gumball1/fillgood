...
class source:
    def __init__(self):
        ...
        self.priority = 1
        self.language = ["pl"]
        self.domains = ["nopremium.pl"]
        self.login_url = "https://www.nopremium.pl/api/rest/login"
        self.search_url = "https://www.nopremium.pl/api/rest/search"
        self.check_url = "https://www.nopremium.pl/api/rest/files/check"
        self.files_url = "https://www.nopremium.pl/api/rest/files/get"
        self.download_url = "https://www.nopremium.pl/api/rest/files/download"
        self.req = requests
        self.user_name = control.settings.getString("nopremium.username")
        self.user_pass = control.settings.getString("nopremium.password")
        self.trash = []
        #self.trash2 = []
        self.titles = []
    ...
    def login(self):
        ...
        self.authtoken = ""
        get_auth = cache.cache_get("nopremium_authtoken", control.sourcescacheFile)
        ...
        if self.authtoken == "":
            if self.user_name and self.user_pass:
                ...
                if "authtoken" in response:
                    ...
                    cache.cache_insert("nopremium_authtoken", self.authtoken, control.sourcescacheFile)
                    return True
                if "error" in response and response["message"] != "":
                    xbmcgui.Dialog().notification("nopremium - Błąd", str(response["message"]), xbmcgui.NOTIFICATION_ERROR)
            else:
                xbmcgui.Dialog().notification("nopremium - Błąd", "Brak danych logowania - sprawdź ustawienia", xbmcgui.NOTIFICATION_ERROR)
        return False
    ...
    def search(self, title, localtitle, year="", episode="", premiered="", aliases=None, recurrency=None):
        """ pobiera wyniki z API serwisu nopremium """
        try:
            ...
            for title in titles:
                ...
                if "error" in search and search["message"] != "":
                    cache.cache_insert("nopremium_authtoken", "", control.sourcescacheFile)
    ...
    def resolve(self, url):
        ...
        try:
            ...
            autonopremium = control.settings.getBool("autonopremium")
            ...
            if "file" in response:
                ...
                if "url" in file:
                    ...
                    control.window.setProperty('clear_SourceCache_for', control.window.getProperty('clear_SourceCache_for') + ',nopremium')  # jak ktoś używa cache źródeł
            ...
            xbmcgui.Dialog().notification(f"nopremium - {error}", message, xbmcgui.NOTIFICATION_ERROR)
            # return "https://www.nopremium.pl/error.mp4"
            return None