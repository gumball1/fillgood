...
class source:
    ...
    def search(self, title, localtitle, year, aliases=None, ep=False):
        ...
        # titles = [localtitle, originalname, title]  # bo nie znajduje serialu "Transplant" (bo Filman dał tylko 1 tytuł angielski)
        ...
        for title in titles:
            ...
            # fflog(f' {" / ".join(self.founds)=} ')
            # fflog(f' {self.founds=} ')
            # if title_r in " / ".join(self.founds):  # aby nie otrzymywać tych samych wyników  (nie sprawdza się dla "Archiwum X")
            if title_r in self.founds:
                fflog(f'zapytanie {title_r=} nie da nowego rezultatu')
                fflog(f' {self.founds=} ')
                # break
                continue
            ...
            if result:
                fflog(f'otrzymano jakieś wyniki', 1)
                item = self.find_item({'title': title, 'url': result, 'year': year, 'quality': None, 'is_ep': ep})
                if item:
                    return item
                else:
                    # fflog(f'nic nie dopasowano')  # ten tekst jest w funkcji "find_item"
                    pass
                # fflog(f'{self.founds=}')
                # fflog(f'{" / ".join(self.founds)=}')
            else:
                fflog(f'nie otrzymano żadnych wyników', 1)
                pass
            ...
            if ( title == titles[-1]  # ostatni
                ...
                i = 1
                for a in aliases2:
                    if a.lower() not in titles:  # jeszcze może z "&" zamienionego na "&amp;" trzeba by sprawdzać
                        titles += [a.lower()]  # dodanie
                        if i >= 4:  # ograniczenie
                            break
                        i += 1
                # transformacja taka sama, jak była dla początkowych tytułów
                titles = list(filter(None, titles))  # usunięcie pustych
                titles = [t.lower() for t in titles]  # może pomóc pozbyć się duplikatów
                titles = [t.replace("&", "&amp;") for t in titles]
                titles = list(dict.fromkeys(titles))  # pozbycie się duplikatów
            ...
            control.sleep(300)  # na wszelki wypadek (opóźnienie przed kolejnych requestem)
    ...
    def sources(self, url, hostDict, hostprDict):
        ...
        try:
            ...
            for d in data:
                ...
                info2 += f'.{ext}' if ext else ""

                if ext:
                    if ( ext not in info2
                        or not filename_in_2nd_line and extrainfo
                    ):
                        # if d.get('type') == "mp4":
                        info += f" | ({ext})"
                        pass
