...
class source:
    def __init__(self):
        ...
        self.base_url = "https://obejrzyj.to"
    ...
    def sources(self, url, hostDict, hostprDict):
        ...
        if data:
            ...
            for u in videos:
                sources.append({
                    "source": u['name'].split(" ")[0],
                    "quality": u['quality'],
                    "language": u['language'],
                    "url": u['src'] + ("$$subs" if "napisy" in u['name'].lower() else ""),
                    "info": u['name'].replace(u['name'].split(" ")[0], "").replace(u['language'],'').replace(u['language'].upper(),'').replace('  ',' ').strip(),
                    "direct": False,
                    # "direct": u['type'] != 'embed',  # to może przyszłościowo, bo na razie nie trafiłem na inny typ
                    "debridonly": False,
                })
