...
class source:
    ...
    def movie(self, imdb, title, localtitle, aliases, year):
        try:
            ...
            log_utils.log("putlockersnet Exception", "sources")
            return
    ...
    def scrapeGomo(self, url, hostDict):
        ...
        try:
            ...
            if tc:
                ...
                result = ensure_text(client.request(self.gomo_link, XHR=True, post=post, headers=headers),
                    errors="replace", )
                ...
                for link in links:
                    ...
                    if "gomo.to" in link:
                        ...
                        if "gomoplayer.com" in link:
                            sourcePage = ensure_text(requests.get(link, headers=self.headers).content,
                                errors="ignore", )
                            if jsunpack.detect(sourcePage):
                                ...
                                for url in urls:
                                    ...
                                    sources.append(
                                        {"source": "CDN", "quality": "SD", "language": "en", "url": url, "info": info,
                                            "direct": False,
                                            "debridonly": False, })  # https://database.gdriveplayer.us/player.php?imdb=tt1825683
                    else:
                        valid, host = source_utils.is_host_valid(link, hostDict)
                        if valid:
                            quality, info = source_utils.get_release_quality(link, link)
