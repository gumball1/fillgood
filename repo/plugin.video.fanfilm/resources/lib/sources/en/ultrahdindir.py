...
class source:
    ...
    def movie(self, imdb, title, localtitle, aliases, year):
        try:
            ...
            log_utils.log("ultrahd_exc3", "sources")
            return
        ...
        try:
            ...
            for item in r:
                try:
                    ...
                    try:
                        details = client.parseDOM(data, "div", attrs={"class": "text_spoiler"})[0]
                    except:
                        details = None
                    if details:
                        _zip = zip([u for u in urls if u.startswith("https://turbobit")],
                            re.findall(r"General : (.+?)<br", details), re.findall(r"Length : (.+?) for", details), )
                    else:
                        _zip = zip([u for u in urls if u.startswith("https://turbobit")],
                            re.findall(r"/uploads/0-0-vip-(.+?).jpg", data, re.I | re.S), )
                    ...
                    for z in _zip:
                        try:
                            ...
                            if quality == "sd" and "remux" in name.lower():
                                quality = "1080p"
                            ...
                            # continue
                            sources.append(
                                {"source": "turbobit", "quality": quality, "language": "en", "url": url, "info": info,
                                    "size": dsize, "name": name, "direct": False, "debridonly": True, })
                        except:
                            log_utils.log("ultrahd_exc2", "sources")
                            pass
                except:
                    log_utils.log("ultrahd_exc1", "sources")
                    pass
            return sources
        except:
            log_utils.log("ultrahd_exc0", "sources")
            return sources
