...
class source:
    def __init__(self):
        ...
        self.user_name = control.addon("plugin.video.fanfilm").getSetting("furk.user_name")
        self.user_pass = control.addon("plugin.video.fanfilm").getSetting("furk.user_pass")
        self.api_key = control.addon("plugin.video.fanfilm").getSetting("furk.api")
        self.search_limit = control.addon("plugin.video.fanfilm").getSetting("furk.limit")
        self.files = []
        ...
        try:
            ...
            if api_key == "":
                if self.user_name == "" or self.user_pass == "":
                    ...
                    if p["status"] == "ok":
                        api_key = p["api_key"]
                        control.addon("plugin.video.fanfilm").setSetting("furk.api", api_key)
                    else:
                        pass
            ...
            for i in files:
                if i["is_ready"] == "1" and i["type"] == "video":
                    try:
                        ...
                        sources.append(
                            {"source": source, "quality": quality[0], "language": "en", "url": url, "info": details,
    ...
    def resolve(self, url):
        ...
        try:
            ...
            if self.content_type == "episode":
                self.filtering_list = self.seasEpQueryList(info[1], info[2])
