...
class source:
    def __init__(self):
        self.priority = 1
        self.language = ["en"]
        self.domains = ["go2.myvideolinks.net", "see.home.kg"]
        self.base_link = "https://go2.myvideolinks.net"
    ...
    def sources(self, url, hostDict, hostprDict):
        ...
        try:
            ...
            query = ("%s S%02dE%02d" % (
            title, int(data["season"]), int(data["episode"])) if "tvshowtitle" in data else "%s %s" % (
            title, data["year"]))
            query = re.sub("(\\\|/| -|:|;|\*|\?|\"|'|<|>|\|)", " ", query)
            ...
            if not "tvshowtitle" in data:
                results = [i for i in results if data["imdb"] in i]
            ...
            for post in posts:
                try:
                    ...
                    if "tvshowtitle" in data:
                        z = zip(re.findall(r"<p><b>(%s.+?)</b>" % title, r, re.I | re.S),
                        ...
                        for f in z:
                            ...
                            t = f[0]
                            try:
                                s = re.findall(r"((?:\d+\.\d+|\d+,\d+|\d+|\d+,\d+\.\d+)\s*(?:GB|GiB|MB|MiB))", t, )[0]
                            except:
                                s = "0"
                        ...
                        u = [i for i in u if "/embed/" not in i]
                        try:
                            s = re.findall(r"((?:\d+\.\d+|\d+,\d+|\d+|\d+,\d+\.\d+)\s*(?:GB|GiB|MB|MiB))", r, )[0]
                        except:
                            s = "0"
                        items += [(t, i, s) for i in u]

                except:
                    log_utils.log("MYVIDEOLINK ERROR", "sources")
                    pass
